...
describe("App", () => {
  ...
  it("displays logged-in account, membership level, and session expiration date beautifully", async () => {
    invokeMock.mockImplementation(async (command: string, args?: { range?: string }) => {
      if (command === "scan_usage") {
        return { importedDays: 3, scannedAt: "2026-04-26T00:00:00.000Z", timezone: "UTC" };
      }
      ...
      if (command === "fetch_codex_limits") {
        return {
          ...
          membershipLevel: "Pro",
          membershipExpiresAt: "2026-06-28T15:49:06.000Z",
        };
      }
      ...
      if (command === "fetch_overview" && args?.range === "30d") {
        return {
          ...
          costDrivers: { projects: [], models: [], dates: [] },
        };
      }
    ...
    // Wait for the email and membership level to render
    await waitFor(() => {
    ...
    // Check that the formatted session expiration date is rendered
    expect(screen.getByText(/Session Expires:.*2026/)).toBeInTheDocument();
  });
});
