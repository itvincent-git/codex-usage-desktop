...
import { AlertTriangle, ChevronLeft, ChevronRight, LoaderCircle, RefreshCw, Search } from "lucide-react";
...
import { fetchModelPricingCatalog, refreshModelPricing, type ModelPricingCatalogResponse, type OverviewResponse, type RangeKey } from "@/lib/api";
...
type ModelsPageProps = {
  ...
  onPricingRefreshed?: () => void | Promise<void>;
};

export function ModelsPage({ models, range, onRangeChange, onPricingRefreshed }: ModelsPageProps) {
  ...
  const refreshCatalog = () => {
    ...
    const request = refreshModelPricing()
      .then(async (nextCatalog) => {
        setCatalog(nextCatalog);
        await onPricingRefreshed?.();
      })
      .catch(() => setError(true))
      .finally(() => {
  ...
  return <div className="space-y-4">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      {view === "usage" ? <RangeSwitcher value={range} onChange={onRangeChange} /> : catalog ? <Button data-testid="refresh-pricing" variant="secondary" disabled={isRefreshing} onClick={refreshCatalog}>{isRefreshing ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}{t("models.catalog.refresh")}</Button> : null}
    </div>

    <div role="tablist" aria-label={t("models.views.label")} className="inline-flex rounded-lg border border-border bg-muted/50 p-1">
      {(["usage", "catalog"] as const).map((item) => <button key={item} id={`models-${item}-tab`} data-testid={`models-${item}-tab`} type="button" role="tab" aria-selected={view === item} aria-controls={`models-${item}-panel`} onClick={() => selectView(item)} className={cn("rounded-md px-3 py-1.5 text-sm font-medium", view === item ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}>{t(`models.views.${item}`)}</button>)}
    </div>
    ...
    {view === "usage" ? <div id="models-usage-panel" role="tabpanel" aria-labelledby="models-usage-tab"><ModelUsageCard models={models} /></div> : <div id="models-catalog-panel" role="tabpanel" aria-labelledby="models-catalog-tab" className="space-y-4" data-testid="pricing-catalog">
      ...
      {catalog ? <div className="flex flex-col gap-3 sm:flex-row">
        <label className="relative flex-1"><span className="sr-only">{t("models.catalog.search")}</span><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input data-testid="pricing-search" value={query} onChange={(event) => { setQuery(event.target.value); resetPage(); }} placeholder={t("models.catalog.search")} className="h-10 w-full rounded-md border border-input bg-background pl-9 pr-3 text-sm outline-none focus:ring-2 focus:ring-ring" /></label>
