...
describe("formatResetTime", () => {
  ...
  it("returns formatted weekly limit style for windowMinutes > 300", () => {
    ...
    const resetDate = dayjs(resetsAtStr).format("YYYY-MM-DD");
    ...
    expect(formatResetTime(resetsAtStr, 10080)).toBe(`Reset ${resetDate} (${daysLeftText})`);
  });
  ...
  it("returns formatted weekly limit style with 1 day left", () => {
    ...
    const resetDate = dayjs(resetsAtStr).format("YYYY-MM-DD");
    expect(formatResetTime(resetsAtStr, 10080)).toBe(`Reset ${resetDate} (24 hours left)`);

    const resetsAtStr2 = "2026-05-23T12:01:00.000Z";
    const resetDate2 = dayjs(resetsAtStr2).format("YYYY-MM-DD");
    expect(formatResetTime(resetsAtStr2, 10080)).toBe(`Reset ${resetDate2} (1 day left)`);
  });
  ...
  it("returns formatted weekly limit style with hours left", () => {
    ...
    const resetDate = dayjs(resetsAtStr).format("YYYY-MM-DD");
    expect(formatResetTime(resetsAtStr, 10080)).toBe(`Reset ${resetDate} (5 hours left)`);
  });
  ...
  it("returns formatted weekly limit style with minutes left", () => {
    ...
    const resetDate = dayjs(resetsAtStr).format("YYYY-MM-DD");
    expect(formatResetTime(resetsAtStr, 10080)).toBe(`Reset ${resetDate} (35 mins left)`);
  });
});
